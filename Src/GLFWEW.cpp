...
namespace GLFWEW {
	...
	*/
	bool Window::Init(int w, int h, const char* title)
	{
		...
		const GLubyte* vendor = glGetString(GL_VENDOR);//実行環境のOpenGL実装について、責任を持つ会社名
		std::cout << "Vendor:" << vendor << std::endl;
		const GLubyte* renderer = glGetString(GL_RENDERER);//OpenGLを描画するグラフィックハードウェア名、またはドライバ名
		std::cout << "Renderer:" << renderer << std::endl;
		const GLubyte* version = glGetString(GL_VERSION);//グラフィックスハードウェア、またはドライバが対応している最大のOpenGLバージョン
		std::cout << "Version:" << version << std::endl;
		/*
		const GLubyte* extensions = glGetString(GL_EXTENSIONS);//グラフィックスハードウェア、またはドライバが対応しているOpenGL拡張名を、スペースで区切って並べたもの
	...
	* 経過時間を取得する
	*
	* @return	直前の2回のUpdateTimer()を呼び出しの間に経過した時間
	*/
	...
	*/
	...
	{
		//buttonDownを生成する更新前の入力状態を保存しておく.
		...
		//両方の配列がnullptrでなく、最低限必要なデータ数を満たしていなければ、
