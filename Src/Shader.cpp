...
namespace Shader {
	...
	*/
	void Program::Reset(GLuint programId)
	{
		...
		locAmbiLightCol = glGetUniformLocation(id, "ambientLight.color");
	...
	/**
	* プログラム・オブジェクトが設定されているか
	* 
	* @retval true		設定されていない
	* @retval false		設定されてされている
	*/
	...
	*/
	void Program::SetLightList(const LightList& lights)
	{
		...
		//ライトの色情報をGPUメモリに転送する
		if (locAmbiLightCol >= 0) {
			glUniform3fv(locAmbiLightCol, 1, &lights.ambient.color.x);
		}
		if (locDirLightCol >= 0) {
	...
	*/
		...
		const glm::vec3& translate, const glm::vec3& rotate, const glm::vec3& scale)
	{
		...
		//モデル座標系におけるスポットライトの座標を変換し、GPUメモリに転送する
